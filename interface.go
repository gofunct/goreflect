...
func IsEmptyDefaultIfEmpty(d interface{}, given ...interface{}) interface{} {
...
func ValueIsValid(i interface{}) bool {
	t := reflect.ValueOf(i)
	switch {
	case t.IsValid() && !t.IsNil() && t.CanAddr() && t.CanSet():
		return true
	default:
		return false
	}
...
func Implements(this interface{}, that interface{}) bool {
	me := reflect.TypeOf(this)
	you := reflect.TypeOf(that)
	switch {
	case me.Implements(you):
		return true
	default:
		return false
	}
...
func PackagePath(object interface{}) string {
	t := reflect.TypeOf(object)
	return t.PkgPath()
}

// toFloat64 converts 64-bit floats
func eToFloat64(v interface{}) float64 {
...
func eToInt(v interface{}) int {
	//It's not optimal. Bud I don't want duplicate toInt64 code.
	return int(eToInt64(v))
}

// toInt64 converts integer types to 64-bit integers
func eToInt64(v interface{}) int64 {
	if str, ok := v.(string); ok {
...
func Slice(list interface{}, indices ...interface{}) interface{} {
	...
	case reflect.Slice, reflect.Array:
		...
		var start, end int
		if len(indices) > 0 {
			start = eToInt(indices[0])
		}
		if len(indices) < 2 {
			end = l
		} else {
			end = eToInt(indices[1])
		}
...
func ToCaseInsensitiveValue(value interface{}) interface{} {
	switch v := value.(type) {
	case map[interface{}]interface{}:
		value = CopyAndInsensitiviseMap(ToStringMap(v))
	case map[string]interface{}:
...
func InsensitivizeMap(m map[string]interface{}) {
	for key, val := range m {
		switch val.(type) {
		case map[interface{}]interface{}:
